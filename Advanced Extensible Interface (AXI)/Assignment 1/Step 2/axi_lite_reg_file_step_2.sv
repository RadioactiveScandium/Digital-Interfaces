...
// Deltas w.r.t Step 1 :
//
//   * Added address decoding/matching feature to write to reg-banks independently  without causing address slippage
//   * Implemented the Read FSM
//   * Added two new registers to demonstrate Read-Only and Write-Only scenarios and mapped them to Read Data bus
//
...
//
// Writing to the registers when the adress and data arrive at
// the same time is not a problem at all. 
// The top level address s_axi_awaddr can be compared against the address map 
// combinationally to perform address matching and use it as a qualifier to
...
// Why aw_hs_done cannot be used as an enable twhile flopping awaddr
...
//    3. Address Matches 
`ASYNC_RESET_EN_FF(reg_bank0, s_axi_wdata, ((state == IDLE) && (aw_hs_done_reg && w_hs_done_reg) && reg_bank0_w_addr_match), aclk, aresetn)
`ASYNC_RESET_EN_FF(reg_bank1, s_axi_wdata, ((state == IDLE) && (aw_hs_done_reg && w_hs_done_reg) && reg_bank1_w_addr_match), aclk, aresetn)
...
always_comb begin : READ_STATE_FSM
   // DEFAULT VALUES (Avoids Latch Inference)
    ...
    case(state_for_read)
        ...
        // 
        READ_DATA : begin
            s_axi_rdata  = reg_bank0_r_addr_match ? reg_bank0 : (reg_bank1_r_addr_match ? reg_bank1 : (reg_bank3_r_addr_match ? hw_status_in : s_axi_rdata));
            s_axi_rresp  = 2'b00;
            s_axi_rvalid = 1'b1;
